...
| `dist/` | 打出来的 `.dusk` 成品包（地区 x 开源字体版 / 官方字库版） |
...
python scripts/build_sjis_pack.py          # 打包 → dist/yiga_zh_hans_<region>.dusk（3 个地区 x 2 个变体）

# 3) 装进游戏（拷包 + 改配置开关）
python scripts/install.py --region us      # --variant ique 装官方字库版；--list 只看现状
```
...
2. `python scripts/install.py --region <us|eu|jp>` 装包（`--variant ique` 装官方字库版）；也可以手工把
...
每个地区两个变体覆盖同一批资源、只能装一个：`dist/yiga_zh_hans_<region>.dusk`（开源字体渲染）与
`dist/yiga_zh_hans_ique_<region>.dusk`（保留官方字库位图）；换地区或换变体装新的会自动清掉旧的。
中文名字键盘与键盘补全只对 open 变体成立；用上游 dusklight 时没有中文键盘（那是 dusk-cn 分支的功能）。
...
- **开源字体**：open 变体的字形由思源黑体 / Noto Sans CJK SC 与霞鹜文楷（均 OFL-1.1）渲染，字体文件需自行下载、
