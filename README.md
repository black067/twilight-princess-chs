...
> 自己做一份本地化 mod 的步骤见 [docs/构建指南.md](docs/构建指南.md)。

## 文档

| 文档 | 内容 |
| --- | --- |
| [docs/构建指南.md](docs/构建指南.md) | 克隆仓库到打出包的步骤、译文表格式、打包命令 |
| [docs/实现细节.md](docs/实现细节.md) | 素材与产物的形态、配置分层、三条打包路线、容器格式与文本语法、码位与键盘、`install.py` 行为、常见坑 |
| [docs/发布指南.md](docs/发布指南.md) | 上传 Dusklight mod 站的字段规则与手工项 |

## 许可与免责

- **本仓库只有脚本与文档**：输入素材（`input/`）与产物（`dist/`、`work/`）请通过合法途径自行获取；
  输入格式见 [docs/实现细节.md](docs/实现细节.md)。
