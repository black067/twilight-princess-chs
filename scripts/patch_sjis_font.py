...
def name_keyboard_aliases(orig_glyph):
    """引擎名字键盘字表（l_mojiZh）是 Shift-JIS 码位，本字库用不上。

    原字不在字库的：55 个日式写法换成对应简体字，其余 102 格指向空白字形。
    """
    ...
    print("   名字键盘别名: %d 格 (原字 %d / 换简体 %d / 空白 %d)"
          % (len(aliases), direct, variant, blanked))
    return aliases


def add_aliases(body, blocks, orig_glyph, remap):
    target = None
    for b in blocks:
    ...
    aliases = name_keyboard_aliases(orig_glyph)
    entries += aliases
    entries.sort()
