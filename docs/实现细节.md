...
flowchart LR
    ...
    EXP -->|"{lang 的 src_file}（整表重写）"| SRC[原文表]
    SRC -->|"抄 key / 对照"| TR[翻译]
    TR -->|"{lang 的 locale_file}"| TA1[从零生成：文本]
    TR -->|"{lang 的 locale_file}"| TA3
    ...
    TA1 -->|"work/{lang}/parts.open.own/*.arc"| PK
    FT1 -->|"work/{lang}/parts.open.own/*.arc"| PK
    TA3 -->|"work/{lang}/parts.open.sjis/*.arc"| PK
    FT3 -->|"work/{lang}/parts.open.sjis/*.arc"| PK
...
[构建指南](构建指南.md)中包含了具体的操作方式

## 素材与源文本

素材目录使用`config.default.json`的`input_dir`进行配置，默认 `input/`

消息库与参照字库需要放进此目录；译文表与原文表的位置由本地化方案的 `locale_file` / `src_file` 决定，缺省都落在这个目录下
...
| `texts.<lang>.csv` / `texts.<lang>.src.csv`                  | 译文表（`locale_file`）与原文表（`src_file`），缺省都落在这里，见[译文与索引](#译文与索引)                                         |
...
源文本在 BMG 里：消息正文在 `DAT1`，单位表的正文在 `STR1` 池。编码由文件自己声明，写在头 0x10：`1` 是每字节一个
...
译文表（`langs.<lang>.locale_file`，缺省 `input/texts.<lang>.csv`）是之后唯一要维护的东西。打包只读 `key` 与译文列 `locale_col`；
多余列一概不看，所以原文列、`comment` 留在译文表里也影响不到打包。

表格结构：译文表 `key,<locale_col>[,其它列]`；原文表 `key,<src_col>[,comment]`。列名和源素材目录都填到配置文件的本地化方案里

两张表都从源素材派生，分两步：先算索引，再导出原文表
...
- 导出时一条文本怎么对应到字段，看 `data/text_resources.json` 里的 `"shape"` 写的是哪种：
  ...
  - `string_pairs`（单位表 `zel_unit`）：条目里的两个 u16 字段是 `STR1` 的**字节偏移**，条目号 = 条目下标，如 `zel_unit/0/one`、
    ...
    插进消息串。
- **注意**，导出只重写原文表（`src_file`），整表覆盖；译文表由人维护，导出脚本不碰它

表里一行怎么写：

- 一行就是一条需要本地化的文本；`key`的结构：`<资源>/<条目>[/<字段>]`，资源名指的是不含后缀的 BMG 文件名（去掉 `.bmg`）
  消息表的条目号是消息号；
  同一消息号出现两次时，后一条补 `#<下标>`。原归档里真有这种：`zel_04` 有1607 条，末条与第 613 条同号且文本为空，引擎只会查到前一条。
- 字面语法
...
| `parts.*/build.json`         | 该资源目录的产物记录：`lang` / `variant` / `code_space` 与各输入文件的 sha256 |
...
| ------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `source`                     | 源素材目录，相对仓库根                                                                                                                                      |
| `src_file` / `src_col`     | 原文表（校对底本）的路径与原文列；`export_texts.py` 写它，origin 变体的校验读它
| `locale_file` / `locale_col` | 译文表（打包输入）的路径与译文列。打包只读译文列                                                                                                                     |
| `default_name_chars`         | 要改写成单字节码位的默认名字表。留空则忽略，见[引擎侧的硬约束](#引擎侧的硬约束)                                                                              |
| `default_name_cells`         | 哪几条按单字节码位写，如`["zel_00/898", "zel_00/899"]`。文本取自译文表                                                                                    |
| `input_dir`（顶层键）        | 本地素材根，缺省`input/`：消息库与参照字库在它下面，`font_source` / `locale_file` / `src_file` 的缺省值也由它推。写在本地化方案里无效                                                    |
...
- 码位方案（命令行 `--code-space`）：`own` 自己分配，配从零生成的字库；`sjis` 沿用 `work/<lang>/sjis_map.json` 里现有字库的码位。
  局限见[生成资源](#生成资源)。
- 加一段新语言：在 `config.default.json` 里新增一个项目，字段是 `source` / `src_file` / `src_col` / `locale_file` /
  `locale_col` / `discs` / `mod` / `mod_origin`，再重算索引、导出原文表，译文表自己建。
...
| 路径                | 内容                                                                                                                                                    |
| ------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `scripts/`        | 构建与打包管线：Python，只用标准库，外部路径写在`config.default.json` / `config.local.json` 里 |
| `data/`           | 存放配置表与索引                                                                                                                                          |
| `docs/`           | 构建指南 / 实现细节 / 发布指南                                                                                                                          |
