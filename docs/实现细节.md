...
译文表 `input/texts.<lang>.csv` 是之后唯一要维护的东西。打包只读 `key` 与译文列 `locale_col`；

表格结结构：`key,<draft_col>,<locale_col>[,comment]`，列名和源素材目录都填到配置文件的本地化方案里

这张表可使用源素材导出，分两步：先算索引，再导出
...
- 导出时一条文本怎么对应到字段，看 `data/text_resources.json` 里的 `"shape"` 写的是哪种：
  ...
  - `string_pairs`（单位表 `zel_unit`）：条目里的两个 u16 字段是 `STR1` 的**字节偏移**，条目号 = 条目下标，如 `zel_unit/0/one`、
    ...
    插进消息串。
- **注意**，导出是整表重写：译文列与底稿列都会直接写入表格，要保留的译文先备份
